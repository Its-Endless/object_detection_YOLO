...
engine = pyttsx3.init()
def announce_object(object_name, direction):
    engine.say(f"{object_name} detected {direction}")
    engine.runAndWait()
...
while True:
    ...
    for box in predictions:
        ...
        if object_name in target_objects:
            ...
            current_frame_objects.add((object_name, direction))

    for obj, direction in current_frame_objects:
        if obj not in detected_objects_in_frame:
            announce_object(obj, direction)

    detected_objects_in_frame = {obj for obj, _ in current_frame_objects}
...
#DISTANCE IS ALSO CALCULATED IN THE BELOW CODE
...
#     k = 1000  # Example constant for calibration
#     c = 0.5   # Offset constant
#     box_height = box_height.item()  # Convert Tensor to a scalar
